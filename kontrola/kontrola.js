...
function renderMedia(){document.querySelectorAll('[data-media-list]').forEach(el=>{const a=get(el.dataset.mediaList)||[];el.innerHTML=a.map(n=>'<span class="photo-pill">'+esc(n)+'</span>').join('')})}
...
function renderFiles(){const p=document.querySelector('#photoList');if(p)p.innerHTML=(get('photos.names')||[]).map(n=>'<span class="photo-pill">'+esc(n)+'</span>').join('');const d=document.querySelector('#diagList');if(d)d.innerHTML=(get('diagnostics.files')||[]).map(n=>'<span class="photo-pill">'+esc(n)+'</span>').join('');const docs=document.querySelector('#docsList');if(docs)docs.innerHTML=(get('documents.files')||[]).map(n=>'<span class="photo-pill">'+esc(n)+'</span>').join('')}
function validate(){const missing=[];[['vehicle.make','Značka vozidla'],['vehicle.model','Model vozidla'],['vehicle.vin','VIN'],['vehicle.mileage','Stav km'],['final.verdict','Finální rozsudek'],['final.summary','Celkové hodnocení technika']].forEach(([p,n])=>{if(!get(p))missing.push(n)});const el=document.querySelector('#validationResult');if(!el)return;if(missing.length){el.innerHTML='<div class="validation warnbox"><strong>Před vytvořením reportu zkontrolujte:</strong><ul>'+missing.map(x=>'<li>'+x+'</li>').join('')+'</ul><span>NEOVĚŘENO a NERELEVANTNÍ se nepovažují za chybu.</span></div>'}else{el.innerHTML='<div class="validation okbox"><strong>Základní kontrola je kompletní.</strong><span>V produkční verzi by nyní následovalo vytvoření klientského reportu.</span></div>'}}
document.querySelector('#photoInput').onchange=e=>{set('photos.names',[...e.target.files].map(f=>f.name));renderFiles()};
