...
function engine(){return card('engine','6','Motorový prostor, motor a převodovka','<div class="grid cols-2"><label>Teplota motoru při příjezdu<select data-field="engine.arrivalTemp"><option value="">Vyberte…</option><option>Studený</option><option>Částečně zahřátý</option><option>Zahřátý</option></select></label><label>Studený start<select data-field="engine.coldStart"><option value="">Vyberte…</option><option>Ověřen</option><option>Neověřen</option></select></label></div>'+point('engine.bay','Motorový prostor – vizuální stav')+point('engine.oil','Motorový olej – hladina a vizuální stav')+point('engine.coolant','Chladicí kapalina – hladina a vizuální stav')+point('engine.brakeFluid','Brzdová kapalina – hladina a vizuální stav')+point('engine.otherFluids','Ostatní relevantní provozní kapaliny')+point('engine.battery','Akumulátor – vizuální stav / napětí pouze pokud změřeno')+point('engine.start','Start a chod motoru')+point('engine.warm','Kontrola po zahřátí')+point('engine.belts','Viditelné řemeny a příslušenství – pokud přístupné')+point('engine.gearboxVisual','Převodovka – pouze viditelný fyzický stav'))}
...
function bind(){document.querySelectorAll('[data-field]').forEach(el=>{const p=el.dataset.field;if(el.type==='checkbox'){el.checked=!!get(p);el.onchange=()=>set(p,el.checked)}else{el.value=get(p)??'';el.oninput=()=>set(p,el.value);el.onchange=()=>set(p,el.value)}});document.querySelectorAll('[data-choice] button').forEach(btn=>btn.onclick=()=>{const row=btn.closest('[data-choice]');set(row.dataset.choice,btn.dataset.value);row.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b===btn))});document.querySelectorAll('[data-media]').forEach(inp=>inp.onchange=e=>{set(inp.dataset.media,[...e.target.files].map(fileMeta));rememberPreviewFiles(inp.dataset.media,e.target.files);renderMedia()});document.querySelectorAll('.verdict-picker button').forEach(btn=>btn.onclick=()=>{set('final.verdict',btn.dataset.value);restore()});const add=document.querySelector('#addInvestment');if(add)add.onclick=()=>{const a=get('investments.items')||[];a.push({title:'',note:'',min:'',max:''});set('investments.items',a);renderInvestments();bind();restore();addDictationButtons()};const d=document.querySelector('#diagInput');if(d)d.onchange=e=>{set('diagnostics.files',[...e.target.files].map(fileMeta));rememberPreviewFiles('diagnostics.files',e.target.files);renderFiles()};const docs=document.querySelector('#docsInput');if(docs)docs.onchange=e=>{set('documents.files',[...e.target.files].map(fileMeta));rememberPreviewFiles('documents.files',e.target.files);renderFiles()};renderMedia();addDictationButtons();updateFinalSummary()}
...
function rememberPreviewFiles(path,files){previewFiles[path]=[...files].map(f=>({file:f,url:URL.createObjectURL(f)}))}
function fileCard(meta,preview){const name=fileName(meta),type=(typeof meta==='object'&&meta?.type)||preview?.file?.type||'',url=preview?.url||'';if(url&&type.startsWith('image/'))return '<div class="file-preview-card"><img src="'+url+'" alt=""><span>'+esc(name)+'</span></div>';if(url&&type.startsWith('video/'))return '<div class="file-preview-card"><video src="'+url+'" preload="metadata" muted playsinline></video><b class="video-badge">▶ VIDEO</b><span>'+esc(name)+'</span></div>';const ext=(name.split('.').pop()||'SOUBOR').toUpperCase();return '<div class="file-preview-card document-preview"><b>'+esc(ext)+'</b><span>'+esc(name)+'</span></div>'}
function renderMedia(){document.querySelectorAll('[data-media-list]').forEach(el=>{const path=el.dataset.mediaList,a=get(path)||[],p=previewFiles[path]||[];el.innerHTML=a.map((m,i)=>fileCard(m,p[i])).join('')})}
...
function renderFiles(){[['#photoList','photos.names'],['#diagList','diagnostics.files'],['#docsList','documents.files']].forEach(([sel,path])=>{const el=document.querySelector(sel);if(!el)return;const a=get(path)||[],p=previewFiles[path]||[];el.innerHTML=a.map((m,i)=>fileCard(m,p[i])).join('')})}
...
document.querySelector('#photoInput').onchange=e=>{set('photos.names',[...e.target.files].map(fileMeta));rememberPreviewFiles('photos.names',e.target.files);renderFiles()};
