...
function drive(){const items=[['motor','Motor'],['gearbox','Převodovka / spojka / řazení'],['suspension','Podvozek'],['brakes','Brzdy + parkovací brzda'],['steering','Řízení'],['acceleration','Akcelerace'],['drivetrain','Pohon'],['assist','Asistenční systémy']];return card('drive','11','Zkušební jízda','<div class="safety-note"><strong>Zjištění zapisujte až po zastavení vozidla.</strong><span>Přílohy přidávejte pouze bezpečně po zastavení.</span></div>'+items.map(([k,t])=>'<div class="check-point"><h3>'+t+'</h3>'+status('sections.drive.'+k+'.status')+note('sections.drive.'+k+'.note')+media('sections.drive.'+k+'.media')+'</div>').join(''))}

function videoSummary(){return card('video-summary','12','Video shrnutí kontroly','<p class="muted">Krátké shrnutí kontroly s komentářem technika – pozitivní i negativní zjištění. Doporučená délka přibližně 3–5 minut.</p>'+media('videoSummary.video','＋ Přidat video shrnutí'))}

function investments(){return card('investments','13','Doporučený servis a očekávané investice','<p class="muted">Přidávejte pouze položky, které sami považujete za doporučený servis nebo očekávanou investici. Částky zadává technik; systém je pouze sečte. Jde o orientační odhad, nikoli cenovou nabídku servisu.</p><div id="investmentRows"></div><button type="button" class="ghost" id="addInvestment">+ Přidat položku</button><div class="total-box"><span>OČEKÁVANÉ INVESTICE CELKEM</span><strong id="investmentTotal">0 – 0 Kč</strong></div>')}

function render(){nav();document.querySelector('#dynamicSections').innerHTML=[bodySection(),wheels(),brakes(),engine(),underbody(),interior(),documents(),diagnostics(),drive(),videoSummary(),investments()].join('');renderInvestments();bind();restore();document.querySelector('#inspectionId').textContent=state.meta?.id||makeId();renderFiles();updateInvestmentTotal();updateFinalSummary();addDictationButtons()}
