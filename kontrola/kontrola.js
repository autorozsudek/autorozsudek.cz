...
function engine(){return card('engine','6','Motorový prostor, motor a převodovka',point('engine.bay','Motorový prostor – vizuální stav')+point('engine.oil','Motorový olej – hladina a vizuální stav')+point('engine.coolant','Chladicí kapalina – hladina a vizuální stav')+point('engine.brakeFluid','Brzdová kapalina – hladina a vizuální stav')+point('engine.otherFluids','Ostatní relevantní provozní kapaliny')+point('engine.battery','Akumulátor – vizuální stav')+'<div class="grid cols-2"><label>Teplota motoru při příjezdu<select data-field="engine.arrivalTemp"><option value="">Vyberte…</option><option>Studený</option><option>Částečně zahřátý</option><option>Zahřátý</option></select></label><label>Studený start<select data-field="engine.coldStart"><option value="">Vyberte…</option><option>Ověřen</option><option>Neověřen</option></select></label></div>'+point('engine.start','Start a chod motoru')+point('engine.warm','Kontrola po zahřátí')+point('engine.belts','Viditelné řemeny a příslušenství – pokud přístupné')+point('engine.gearboxVisual','Převodovka – pouze viditelný fyzický stav'))}
